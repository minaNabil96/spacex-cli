"""Integration tests for capsules commands."""
...
@responses.activate
def test_list_capsules_json(capsules_data):
    responses.add(
        responses.GET, 
        f"{BASE}/config/spacecraft/", 
        json={"results": capsules_data, "count": len(capsules_data)}
    )
    result = runner.invoke(app, ["-o", "json", "capsules", "list"])
    assert result.exit_code == 0
...
@responses.activate
def test_capsule_info_not_found():
    responses.add(responses.GET, f"{BASE}/config/spacecraft/bad-id/", status=404)
    result = runner.invoke(app, ["capsules", "info", "bad-id"])
    assert result.exit_code == 1


@responses.activate
def test_capsule_info_json(single_capsule_data):
    capsule_id = single_capsule_data["id"]
    responses.add(responses.GET, f"{BASE}/config/spacecraft/{capsule_id}/", json=single_capsule_data)
    result = runner.invoke(app, ["-o", "json-pretty", "capsules", "info", str(capsule_id)])
    assert result.exit_code == 0
